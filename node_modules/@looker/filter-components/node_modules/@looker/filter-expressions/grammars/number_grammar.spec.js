...
const testNumericItem = testItem => {
...
const numeric = [['1', '=', '1'], ['1, 2, 3', '=', '1,2,3'], ['3.14159', '=', '3.14159'], ['123456789', '=', '123456789'], ['0.01', '=', '0.01'], ['.01', '=', '0.01'], ['-.01', '=', '-0.01'], ['-0.01', '=', '-0.01'], ['1, -1, 0.1', '=', '1,-1,0.1'], ['not 1', '!=', '1'], ['not 1, 2, 3', '!=', '1,2,3'], ['<> 1', '!=', '1'], ['!= 1, 2, 3', '!=', '1,2,3'], ['not -1.2', '!=', '-1.2'], ['not -.2', '!=', '-0.2'], ['> 1.1', '>', '1.1'], ['>0.1', '>', '0.1'], ['>999', '>', '999'], ['> -42', '>', '-42'], ['>-242', '>', '-242'], ['>    0', '>', '0'], ['< 1.1', '<', '1.1'], ['<3', '<', '3'], ['<0.1', '<', '0.1'], ['<999', '<', '999'], ['< -42', '<', '-42'], ['<-242', '<', '-242'], ['<    0', '<', '0'], ['<= 1.1', '<=', '1.1'], ['<=0.1', '<=', '0.1'], ['<=999', '<=', '999'], ['<= -42', '<=', '-42'], ['<=-242', '<=', '-242'], ['<=    0', '<=', '0'], ['>= 1.1', '>=', '1.1'], ['>=0.1', '>=', '0.1'], ['>=999', '>=', '999'], ['>= -42', '>=', '-42'], ['>=-242', '>=', '-242'], ['>=    0', '>=', '0']];

const testNumeric = (expression, type, textInput) => {
  const ast = parseFilterExpression('number', expression);
  expect(ast).toMatchSnapshot();
  const list = treeToList(ast);
  const item = list[0];
...
const nullValues = [['NULL', 'null'], ['NOT NULL', '!null'], ['null', 'null'], ['not null', '!null'], ['nUll', 'null'], ['Not Null', '!null']];

const testNull = (expression, type) => {
  try {
    const ast = parseFilterExpression('number', expression);
    expect(ast).toMatchSnapshot();
    const itemType = convertTypeToOption(ast);
    expect(itemType).toEqual(type);
...
const nowSupported = [["1 to", ">=", "1"], ["to -1", "<=", "-1"], ["to 0.1", "<=", "0.1"], ["not 1, not 2", "!=", "1,2"], ["<> 1, <> 2", "!=", "1,2"], ["!= 1, != 2", "!=", "1,2"], ["1, not 2", "!=", "1,2"], [">1 AND <2 OR >3 AND <4", "between", ">1 AND <2 OR >3 AND <4"]];
describe('nowSupported expressions', () => {
  it.each(nowSupported)('%s', testNumeric);
});
const unsupported = [["0.1.1.1", "matchesAdvanced", "0.1.1.1"], ["0.....1", "matchesAdvanced", "0.....1"], ["--1", "matchesAdvanced", "--1"], ["foo", "matchesAdvanced", "foo"], ["seventeen", "matchesAdvanced", "seventeen"], ["&,,,$%testContext.#,,,$,testContext.", "matchesAdvanced", "&,,,$%testContext.#,,,$,testContext."], ["\\\\\\\\\\\\\\", "matchesAdvanced", "\\\\\\\\\\\\\\"], ["~`!testContext.#$%^*()-+=_{}[]|?", "matchesAdvanced", "~`!testContext.#$%^*()-+=_{}[]|?"], ["<>,. ¡™£¢∞§¶•ªº–≠œ∑", "matchesAdvanced", "<>,. ¡™£¢∞§¶•ªº–≠œ∑"], ["´®†¥¨ˆøπ“‘åß∂ƒ©˙∆˚¬…æ", "matchesAdvanced", "´®†¥¨ˆøπ“‘åß∂ƒ©˙∆˚¬…æ"], ["Ω≈ç√∫˜µ≤≥÷", "matchesAdvanced", "Ω≈ç√∫˜µ≤≥÷"], ["😻🌚", "matchesAdvanced", "😻🌚"], ["^12345", "matchesAdvanced", "^12345"], ["1234^, 567", "matchesAdvanced", "1234^, 567"]];
